...
export class PhoneComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    private attentionMsg: string = 'Вы точно хотите удалить номер телефона?';
    private submitDefaultFormCreate: string = 'Отправить';
    isOpenForms: boolean = false;
    isOpenFormCheck: boolean = false;
    formCreate: FormGroup;
    formCheck: FormGroup;
    isProdMode: boolean = environment.production;
    phones: PhoneInterface[] = [];
    timeLockInputPhone: number = 0;
    ...
    onSubmitFormCreate({target}): void {
        if (this.formCreate.invalid) {
            for (let key in this.formCreate.controls) {
    ...
    onSubmitFormCheck({target}): void {
        if (this.formCheck.invalid) {
            for (let key in this.formCheck.controls) {
