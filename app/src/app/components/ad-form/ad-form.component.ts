...
export class AdFormComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    private ym: any;
    private map: any;
    private defaultCenterMap: number[] = [41.311151, 69.279737];
    ...
    ngOnInit(): void {
        this.formJSON = {
            ...
            cityName: '',
            countryName: '',
            phoneId: [0, [Validators.required, Validators.min(1)]],
            createdAt: '',
            updatedAt: '',
        };
        ...
        const s1 = this.serviceManager.settings$.subscribe(x => {
            this.catsTree = x.catsTree;
            this.catTreeOneLevel = Helpers.getCatTreeAsOneLevel(x.catsTree);
        });
        this.subscriptions.push(s1);

        const s2 = this.serviceAuth.profile$.subscribe(x => {
            this.phones = (x && x.phones && x.phones.length ? x.phones : []); // userExt
        });
    ...
    private responseCat(c: CatFullInterface): void {
        ...
        for (let key in this.form.controls) {
            if (newForm.contains(key)) {
                ...
                if (isPlatformBrowser(this.platformId) && key === 'youtube') {
    ...
    removePhoto({target}, pseudoControlName): void {
        ...
        owner.classList.add('hidden'); // не удаляем явно элемент, чтоб сохранилась индексация эл-ов

        if (!grandFather.querySelectorAll('.form_thumbnails_thumbnail:not(.hidden)').length) {
